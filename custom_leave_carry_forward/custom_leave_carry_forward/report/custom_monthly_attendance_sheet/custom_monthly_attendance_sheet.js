...
frappe.query_reports["Custom Monthly Attendance Sheet"] = {
	filters: [
		{
	...
	onload: function () {
		return frappe.call({
            ...
            method:
	            "custom_leave_carry_forward.custom_leave_carry_forward.report.custom_monthly_attendance_sheet.custom_monthly_attendance_sheet.get_attendance_years",
			callback: function (r) {
				var year_filter =
					frappe.query_report.get_filter("year");

				year_filter.df.options = r.message;
				year_filter.df.default =
					r.message.split("\n")[0];
	...
	formatter: function (
		...
		const group_by =
			frappe.query_report.get_filter_value("group_by");
...
function validate_date_range(report) {
	let start_date =
		frappe.query_report.get_filter_value("start_date");

	let end_date =
		frappe.query_report.get_filter_value("end_date");
	...
	let milli_seconds_in_a_day =
		24 * 60 * 60 * 1000;

	let day_diff = Math.floor(
		(end - start) / milli_seconds_in_a_day
	);

	if (day_diff > 90) {
		frappe.throw({
			message: __(
				"Please set a date range less than 90 days."
			),
